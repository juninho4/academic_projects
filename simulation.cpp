...
void singleInteraction(struct body *a, struct body *b) {
    ...
    // posDiff = (body_a - body_b) * TO_METERS
    vec3 pos_diff;
    pos_diff.x = (a->position.x - b->position.x) * TO_METERS;
    pos_diff.y = (a->position.y - b->position.y) * TO_METERS;
    pos_diff.z = (a->position.z - b->position.z) * TO_METERS;
    ...
    double force = TIME_STEP * (G * a->mass * b->mass) / ((distance * distance + SOFTENING * SOFTENING) * distance);

    // target.accel -= force * posDiff / target.mass
    a->accel.x -= force * pos_diff.x / a->mass;
    a->accel.y -= force * pos_diff.y / a->mass;
    a->accel.z -= force * pos_diff.z / a->mass;

    // other.accel += force * posDiff / other.mass
    b->accel.x += force * pos_diff.x / b->mass;
    b->accel.y += force * pos_diff.y / b->mass;
    b->accel.z += force * pos_diff.z / b->mass;
}
